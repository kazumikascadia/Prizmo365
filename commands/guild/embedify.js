...
module.exports = {
    ...
    async execute(interaction) {
        ...
        // reusing old code from set.js
        if (color !== 'Random ❓ (random color)') {
            const cIndex = color.indexOf('#');
            const cValue = color.slice(cIndex + 1, cIndex + 7);
        ...
        const responseEmbed = new EmbedBuilder()
            .setTitle('Embedify')
            .setDescription('Creating your embed!')
            .setAuthor({ name: nickname, iconURL: avatar })
            .setColor(nColor)
            .setTimestamp(+new Date());

        const embedifyEmbed = new EmbedBuilder()
            .setTitle(title)
        ...
        interaction.channel.send({ embeds: [embedifyEmbed] });
    },
};